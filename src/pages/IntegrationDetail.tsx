      ...
      onSuccess: (changes) => {
        ...
        if (changes.length === 0) {
          ...
          toast.success("Already up to date", {
            description: "No changes from the integration.",
          });
          return;
        }
        ...
        updateStatus(
          integrationId,
          hasConflicts ? "conflict" : "pending_review",
        );

        toast.success("Sync data fetched", {
          description: `${changes.length} changes to review`,
        });
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        {integration.status === "pending_review" ? (
          ...
          >
            <RefreshCw
              className={`w-4 h-4 ${syncMutation.isPending || isSyncing ? "animate-spin" : ""}`}
            />
            Sync Now
          </button>
        )}
              ...
              {recentHistory.map((entry) => (
                ...
                >
                  ...
                  <td className="px-5 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded font-medium ${SOURCE_STYLES[entry.source]}`}
                    >
                      {entry.source}
                    </span>
                  </td>
                  ...
                  <td className="px-5 py-3 text-right">
                    {entry.version && (
                      <Link
                        ...
                        className="inline-flex items-center gap-1 text-sm text-gray-600 hover:underline whitespace-nowrap"
