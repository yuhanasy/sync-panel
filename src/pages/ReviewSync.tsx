  ...
  return (
    <div className="space-y-6">
      ...
      >
        ...
        <div>
          <p
            className={`text-sm font-medium ${counts.conflict > 0 ? "text-amber-800" : "text-blue-800"}`}
          >
            {counts.conflict > 0 ? "Conflicts Detected" : "Approval Required"}
          </p>
          <p
            className={`text-sm mt-0.5 ${counts.conflict > 0 ? "text-amber-700" : "text-blue-600"}`}
          >
            {counts.conflict > 0
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-gray-500">
          {selected.size} of {totalChanges} selected
        </span>
      </div>
          ...
          {Array.from(groupedChanges.entries()).map(([entityType, byId]) => (
            <div key={entityType} className="space-y-3">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-blue-50 border border-blue-200 w-fit">
                {(() => {
                  const Icon =
                    { user: Users, door: DoorOpen, key: Key }[
                      entityType as "user" | "door" | "key"
                    ] ?? Users;
      ...
      <div className="flex items-center justify-between pt-4 border-t border-gray-100">
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 text-sm text-gray-600 hover:text-gray-800 rounded-lg hover:bg-gray-100 transition-colors"
        >
          Cancel Sync
        </button>
        <div className="flex items-center gap-3">
          {(selected.size === 0 || hasUnresolvedConflicts) && (
            <span className="text-sm text-red-600 font-medium">
              {selected.size === 0
                ? "Select at least one change to approve"
                : "Resolve all selected conflicts first"}
            </span>
          )}
          <button
