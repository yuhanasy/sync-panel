...
export function FieldConflict({ change, selected, onToggle, onResolve }: Props) {
    ...
    <div className="space-y-3 py-4 border-b border-gray-100 last:border-0">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <span className={`text-xs font-medium px-2 py-0.5 rounded border ${BADGE_STYLES[change.change_type]}`}>
            {change.change_type}
          </span>
          {isConflict && selected && !isResolved && (
            <span className="text-xs font-medium text-red-600">Resolution required</span>
          )}
        </div>
      </div>
      ...
      {change.change_type === 'ADD' && (
        <div className="ml-7 space-y-1.5">
          ...
          <div className="rounded-lg border border-green-200 bg-green-50 px-3 py-2">
            <p className="text-sm font-mono text-gray-800 break-all">{formatValue(change.new_value)}</p>
          </div>
        </div>
      )}
      ...
      {change.change_type === 'DELETE' && (
        <div className="ml-7 space-y-1.5">
          <span className="text-xs font-medium text-gray-500">Will be removed</span>
          <div className="rounded-lg border border-red-200 bg-red-50 px-3 py-2">
            <p className="text-sm font-mono text-gray-800 break-all line-through">{formatValue(change.current_value)}</p>
          </div>
        </div>
      )}
        ...
        <div className="ml-7 grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            ...
            <div className="rounded-lg border border-gray-200 bg-gray-50 px-3 py-2">
              <p className="text-sm font-mono text-gray-800 break-all">{formatValue(change.current_value)}</p>
            </div>
          </div>
          <div className="space-y-1.5">
        ...
        <div className="ml-7 grid grid-cols-2 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <div className={`rounded-lg border px-3 py-2 ${
              ...
              <p className="text-sm font-mono text-gray-800 break-all">{formatValue(change.local_value)}</p>
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-gray-500">External</span>
              <button
            ...
            <div className={`rounded-lg border px-3 py-2 ${
              ...
              <p className="text-sm font-mono text-gray-800 break-all">{formatValue(change.external_value)}</p>
            </div>
          </div>
        </div>
      )}
